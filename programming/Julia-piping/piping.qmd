...
description: "A short introduction to functional programming in Julia."
jupyter: julia-1.9
reference-location: margin
---

[Advent of code](https://adventofcode.com) is a quaint site that posts one programming problem a day just before Christmas. Just so we can flex our programming muscle. More importanetly, so we can eavsdrop on other people's code. Looking at some of the solutions motivated me to write this tutorial. A lot of people start learning programming with C++ or MATLAB. This inevitably leads to a very verbose and error-prone code. I'd like to show how here how modern programming practices and a good language can help making the solution elegant and maintainable. 
...
We can massage our flow diagram and use Julia's [broadcasting](https://docs.julialang.org/en/v1/manual/arrays/#Broadcasting) mechanism until we get a nice straingt diagram: 
...
There are two important things that happen here. `(first, last)` is a tuple (collection) of functions. 

Julia has many ways of joining things into some form of collection: tuples, named tuples, arrays, dictionaries, structs, …. You can thing of tuples as a glue that joins anything without much boilerplate. They are really usefull when joining things for a moment, just as we need here. 

The dot is the broadcast operator. `.|>` is a way of piping the data into multiple things at once, broadcasting data into individual inputs. 
It works in the same way as `cos.([π,π])`. Here two pies are shoved into a cosine even though cosine function can only handle one pie at a time. 

Now we do have our first and last digits but they still are in a character form. We need to convert them to numbers. As you notice, the output of the previous operation is `('1', '5')` This is a tuple of chars. A `join` function can create a string out of that. 
