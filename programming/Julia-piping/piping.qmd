...
We can massage our flow diagram and use Julia's [broadcasting](https://docs.julialang.org/en/v1/manual/arrays/#Broadcasting) mechanism. 

```{julia}
filter(isdigit, cal_line) .|> (first, last)
```

There are two important things that happen here. `(first, last)` is a tuple of functions. Essentially a collection of functions. 
...
It works in the same way as `cos.([π,π])`. Here two pies are shoved into a cosine even though cosine function can only handle one number at a time. 

```{mermaid}
flowchart LR
  Input(cal_line):::data --> Filter[filter numbers]
  Filter -.-> Numbers[get first and last numbers] -.-> Sum[sum] --> Result(cal_number):::data
  classDef data fill:#C5F4E0
```
